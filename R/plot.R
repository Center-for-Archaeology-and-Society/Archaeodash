...
mainPlot <- function(plotdf, xvar, yvar, attrGroups, Conf, labelPoints, pointLabel, int.set, theme = "viridis") {
  message("main plot")
  print(names(plotdf))
  i <- 1
  # print(head(plotdf))
  ...
  if (shiny::isTruthy(labelPoints)) {
    ...
    ggP <- ggP %>%
      add_text(
        x = xvar,
        y = yvar,
        text = !!as.name(pointLabel),
        textposition = "top right",
        textfont = list(size = 14, color = "black"),
        inherit = FALSE # don't inherit ggplot's default mapping
      )
  }
  print(i)
  i <- i + 1
