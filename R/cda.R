...
getCDA = function(df = rvals$selectedData, chem = rvals$chem, attrGroups = rvals$attrGroups){
  ...
  return(list(mod = mdl_can, CDAdf = dplyr::left_join(df %>% dplyr::select(-tidyselect::any_of(chem)),mdl_can$scores %>% dplyr::mutate(rowid = 1:dplyr::n()) %>% dplyr::select(-tidyselect::any_of(attrGroups)), by = "rowid")))
}
