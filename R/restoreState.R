...
restoreState = function(rvals,input,session){
  inputIDs = c("attr","attrGroups","attrGroupsSub","chem","data.src","xvar","xvar2","yvar","yvar2","plot_theme","Conf","int.set","use_symbols","show_point_labels","pointLabelColumn","runPCA","runLDA","runUMAP")
  types = c("select","select","select","select","select","select","select","select","select","select","checkbox","slider","checkbox","checkbox","select","checkbox","checkbox","checkbox")
